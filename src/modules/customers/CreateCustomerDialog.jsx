...
const getValidationSchema = (isEdit) =>
  Yup.object({
    ...
    role: Yup.string().required("Customer role is required"),
    beneficiaryCount: Yup.number()
      .typeError("Beneficiary count must be a number")
      .integer("Beneficiary count must be an integer")
...
// ── Main Component ─────────────────────────────────────────────────────────────
const CreateCustomerDialog = ({ open, onClose, onSubmit, initialData }) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {isEdit
            ? "Modify the details of this customer below."
        ...
        >
          ...
          <Grid item xs={12} sm={5}>
            <FormControl
              fullWidth
              error={
                formik.touched.beneficiaryCount &&
                Boolean(formik.errors.beneficiaryCount)
              }
            >
              <FieldLabel
              ...
              />
              <InputBase
                value={formik.values.beneficiaryCount}
                onChange={(e) => {
                  const val = e.target.value.replace(/\D/g, "")
                  formik.setFieldValue("beneficiaryCount", val)
                }}
                onBlur={formik.handleBlur("beneficiaryCount")}
                placeholder="e.g. 10"
                inputProps={{ inputMode: "numeric" }}
                sx={{
                  border: `1.5px solid ${
                    formik.touched.beneficiaryCount &&
                    formik.errors.beneficiaryCount
                      ? "#ef4444"
                      : "#e5e7eb"
                  }`,
                  width: "220px",
