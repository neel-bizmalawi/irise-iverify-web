...
const validationSchema = Yup.object({
  trainingSiteName: Yup.string()
    .matches(/^[A-Za-z0-9\s._,'&()\-/]+$/, "Invalid characters")
...
const CreateTrainingSiteDialog = ({ open, onClose, onSubmit, initialData }) => {
  ...
  // Pre-populate selects on edit / reset on close
  useEffect(() => {
    if (open && initialData) {
      if (initialData.district && initialData.districtName) {
        setSelectedDistrict({
          district_id: initialData.district,
          district_name: initialData.districtName,
        });
        fetchAuthorities({ id: initialData.district });
      } else {
        setSelectedDistrict(null);
      }

      if (
        initialData.traditionalAuthority &&
        initialData.traditionalAuthorityName
      ) {
        setSelectedAuthority({
          authority_id: initialData.traditionalAuthority,
          authority_name: initialData.traditionalAuthorityName,
        });
      } else {
  ...
  const searchAuthority = async (query) => {
    if (!selectedDistrict?.district_id) return;
    try {
      setAuthorityLoading(true);
      const res = await axios.get(
        `${API_BASE_URL}/training-site/authority_slug/{district_id}`,
        {
          params: { search: query },
        },
      );
  ...
  const createDistrict = async (name) => {
    ...
    try {
      ...
      // ✅ Use third arg `true` so Yup validates immediately with the new value
      formik.setFieldValue("district", newDistrict?.district_id, true);
    } catch (error) {
      console.error("Add district error:", error);
    }
  };

  // ── Whether the authority field is locked (no district selected yet) ──────
  const authorityLocked = !selectedDistrict?.district_id;
        ...
        <Grid container spacing={2.5}>
          ...
          <Grid item xs={12} md={6}>
            ...
            >
              ...
              <SearchableCreatableSelect
                ...
                onChange={(val) => {
                  // ✅ FIX 1: Pass `true` as third arg so Yup re-validates with the
                  // new value synchronously — this clears the "District is required"
                  // error as soon as a district is picked.
                  ...
                  // Clear authority when district changes
                  setSelectedAuthority(null);
                  formik.setFieldValue("traditionalAuthority", "");
                  setAuthorityOptions([]);
                  // Load authorities for newly selected district
                  if (val?.district_id) {
                    fetchAuthorities({ id: val.district_id });
                  }
                }}
          ...
          <Grid item xs={12} md={6}>
            ...
            >
              ...
              {/* ✅ FIX 2: Wrapper dims + blocks the field until a district is chosen */}
              <Box
                sx={{
                  position: "relative",
                  opacity: authorityLocked ? 0.45 : 1,
                  pointerEvents: authorityLocked ? "none" : "auto",
                  transition: "opacity 0.2s",
                }}
                title={
                  authorityLocked ? "Please select a district first" : undefined
                }
              >
                <SearchableCreatableSelect
                  label={
              ...
              {/* Helper text: show "select district first" hint OR the validation error */}
              {authorityLocked ? (
...
//     .required("Training site name is required"),
//   district: Yup.string().required("District is required"),
//   traditionalAuthority: Yup.string().required("Traditional authority is required"),
//   groupVillageHead: Yup.string()
//     .matches(/^[A-Za-z\s]+$/, "Only letters are allowed")
//     .required("Group village head is required"),
//   villageHeadName: Yup.string()
...
// const StyledInput = ({ value, onChange, onBlur, type = "text", placeholder, error }) => (
//   <InputBase
//     type={type}
//     value={value}
//     onChange={onChange}
...
//   // FIX: selectedDistrict and selectedAuthority hold the full object {id, name}
//   // so SearchableCreatableSelect can display the label on edit
//   const [selectedDistrict, setSelectedDistrict] = useState(null);
//   const [selectedAuthority, setSelectedAuthority] = useState(null);
...
//   // FIX: When editing, pre-populate selectedDistrict and selectedAuthority
//   // from initialData so the select displays the name instead of being blank.
//   useEffect(() => {
//     if (open && initialData) {
//       // initialData carries district (id) and district_name from the parent's handleEdit
//       if (initialData.district && initialData.districtName) {
//         setSelectedDistrict({
//           district_id: initialData.district,
//           district_name: initialData.districtName,
//         });
//         // Also load authorities for the pre-selected district
//         fetchAuthorities({ id: initialData.district });
//       } else {
//         setSelectedDistrict(null);
//       }

//       if (initialData.traditionalAuthority && initialData.traditionalAuthorityName) {
//         setSelectedAuthority({
//           authority_id: initialData.traditionalAuthority,
//           authority_name: initialData.traditionalAuthorityName,
//         });
...
//   // Load authorities when selected district changes
//   useEffect(() => {
//     const id = selectedDistrict?.district_id;
//     if (id) fetchAuthorities({ id });
//   }, [selectedDistrict?.district_id]);

//   const fetchDistricts = async (search = "") => {
//     try {
//       setDistrictLoading(true);
//       const res = await axios.get(`${API_BASE_URL}/training-site/district_slug`, {
//         params: { search },
//       });
//       setDistrictOptions(res.data?.data || []);
//     } catch (error) {
...
//     try {
//       setDistrictLoading(true);
//       const res = await axios.get(`${API_BASE_URL}/training-site/search-district`, {
//         params: { search: query },
//       });
//       setDistrictOptions(res.data?.data || []);
//     } catch (error) {
...
//     try {
//       setAuthorityLoading(true);
//       const res = await axios.get(`${API_BASE_URL}/training-site/search-authority`, {
//         params: { search: query },
//       });
//       setAuthorityOptions(res.data?.data || []);
//     } catch (error) {
...
//       setDistrictOptions((prev) => [...prev, newDistrict]);
//       setSelectedDistrict(newDistrict);
//       formik.setFieldValue("district", newDistrict?.district_id);
//     } catch (error) {
//       console.error("Add district error:", error);
//     }
//   };
...
//         <Typography
//           variant="h6"
//           sx={{ fontWeight: 700, color: "#ffffff", fontSize: "1.2rem", letterSpacing: "-0.01em" }}
//         >
//           {isEdit ? "Update Training Site" : "Create Training Site"}
//         </Typography>
//         <Typography
...
//       {/* Form Body */}
//       <DialogContent sx={{ px: { xs: 2.5, sm: 4 }, py: 3.5, background: "#f8fafc" }}>
//         <Grid container spacing={2.5}>
//           {/* Training Site Name */}
//           <Grid item xs={12} md={6}>
//             <FormControl
//               fullWidth
//               error={formik.touched.trainingSiteName && Boolean(formik.errors.trainingSiteName)}
//             >
//               <FieldLabel icon={LocationOnOutlinedIcon} label="Training Site Name" required />
//               <StyledInput
//                 value={formik.values.trainingSiteName}
//                 onChange={formik.handleChange("trainingSiteName")}
//                 onBlur={formik.handleBlur("trainingSiteName")}
...
//                 error={formik.touched.trainingSiteName && Boolean(formik.errors.trainingSiteName)}
//               />
//               {formik.touched.trainingSiteName && formik.errors.trainingSiteName && (
//                 <FormHelperText>{formik.errors.trainingSiteName}</FormHelperText>
//               )}
//             </FormControl>
//           </Grid>
...
//               error={formik.touched.district && Boolean(formik.errors.district)}
//             >
//               <FieldLabel icon={LocationOnOutlinedIcon} label="District" required />
//               <SearchableCreatableSelect
//                 label="Search District"
//                 value={selectedDistrict}
//                 error={formik.touched.district && Boolean(formik.errors.district)}
...
//                 onSearch={searchDistrict}
//                 onChange={(val) => {
//                   formik.setFieldValue("district", val?.district_id ?? "");
...
//               }
//             >
//               <FieldLabel icon={AccountBalanceOutlinedIcon} label="Traditional Authority" required />
//               <SearchableCreatableSelect
...
//                 onSearch={searchAuthority}
//                 onChange={(val) => {
//                   formik.setFieldValue("traditionalAuthority", val?.authority_id ?? "");
...
//                 allowCreate={false}
//               />
//               {formik.touched.traditionalAuthority && formik.errors.traditionalAuthority && (
//                 <FormHelperText>{formik.errors.traditionalAuthority}</FormHelperText>
//               )}
//             </FormControl>
//           </Grid>
...
//             <FormControl
//               fullWidth
//               error={formik.touched.groupVillageHead && Boolean(formik.errors.groupVillageHead)}
//             >
//               <FieldLabel icon={PersonOutlineOutlinedIcon} label="Group Village Head" required />
//               <StyledInput
//                 value={formik.values.groupVillageHead}
//                 onChange={formik.handleChange("groupVillageHead")}
//                 onBlur={formik.handleBlur("groupVillageHead")}
...
//                 error={formik.touched.groupVillageHead && Boolean(formik.errors.groupVillageHead)}
//               />
//               {formik.touched.groupVillageHead && formik.errors.groupVillageHead && (
//                 <FormHelperText>{formik.errors.groupVillageHead}</FormHelperText>
//               )}
//             </FormControl>
//           </Grid>
...
//             <FormControl
//               fullWidth
//               error={formik.touched.villageHeadName && Boolean(formik.errors.villageHeadName)}
//             >
//               <FieldLabel icon={PersonOutlineOutlinedIcon} label="Village Head Name" required />
//               <StyledInput
//                 value={formik.values.villageHeadName}
//                 onChange={formik.handleChange("villageHeadName")}
//                 onBlur={formik.handleBlur("villageHeadName")}
...
//                 error={formik.touched.villageHeadName && Boolean(formik.errors.villageHeadName)}
//               />
//               {formik.touched.villageHeadName && formik.errors.villageHeadName && (
//                 <FormHelperText>{formik.errors.villageHeadName}</FormHelperText>
//               )}
//             </FormControl>
//           </Grid>
...
//             <FormControl
//               fullWidth
//               error={formik.touched.totalCookstoves && Boolean(formik.errors.totalCookstoves)}
//             >
//               <FieldLabel icon={LocalFireDepartmentOutlinedIcon} label="Total Cookstoves" required />
//               <StyledInput
//                 type="number"
//                 value={formik.values.totalCookstoves}
//                 onChange={formik.handleChange("totalCookstoves")}
...
//                 error={formik.touched.totalCookstoves && Boolean(formik.errors.totalCookstoves)}
//               />
//               {formik.touched.totalCookstoves && formik.errors.totalCookstoves && (
//                 <FormHelperText>{formik.errors.totalCookstoves}</FormHelperText>
//               )}
//             </FormControl>
//           </Grid>
...
//             <FormControl
//               fullWidth
//               error={formik.touched.totalHouseHolds && Boolean(formik.errors.totalHouseHolds)}
//             >
//               <FieldLabel icon={HomeOutlinedIcon} label="Total Households" required />
//               <StyledInput
//                 type="number"
//                 value={formik.values.totalHouseHolds}
//                 onChange={formik.handleChange("totalHouseHolds")}
...
//                 error={formik.touched.totalHouseHolds && Boolean(formik.errors.totalHouseHolds)}
//               />
//               {formik.touched.totalHouseHolds && formik.errors.totalHouseHolds && (
//                 <FormHelperText>{formik.errors.totalHouseHolds}</FormHelperText>
//               )}
//             </FormControl>
//           </Grid>
...
//             <FormControl
//               fullWidth
//               error={formik.touched.totalPeople && Boolean(formik.errors.totalPeople)}
//             >
//               <FieldLabel icon={PeopleAltOutlinedIcon} label="Total People" required />
//               <StyledInput
//                 type="number"
//                 value={formik.values.totalPeople}
//                 onChange={formik.handleChange("totalPeople")}
...
//             <FormControl
//               fullWidth
//               error={formik.touched.houseHoldRadius && Boolean(formik.errors.houseHoldRadius)}
//             >
//               <FieldLabel icon={RadarOutlinedIcon} label="Household Radius (km)" required />
//               <StyledInput
//                 type="number"
//                 value={formik.values.houseHoldRadius}
//                 onChange={formik.handleChange("houseHoldRadius")}
...
//                 error={formik.touched.houseHoldRadius && Boolean(formik.errors.houseHoldRadius)}
//               />
//               {formik.touched.houseHoldRadius && formik.errors.houseHoldRadius && (
//                 <FormHelperText>{formik.errors.houseHoldRadius}</FormHelperText>
//               )}
//             </FormControl>
//           </Grid>
...
//                 row
//                 value={formik.values.roadAccess}
//                 onChange={(e) => formik.setFieldValue("roadAccess", e.target.value)}
//                 sx={{ gap: 1 }}
//               >
//                 {["Yes", "No"].map((opt) => (
//                   <FormControlLabel
...
// //               <SearchableCreatableSelect
// //                 label="Search Authority"
// //                 value={formik.values.AuthorityLoading}
// //                 error={
// //                   formik.touched.traditionalAuthority &&
// //                   Boolean(formik.errors.traditionalAuthority)
// //                 }
